...
public class MultigramGraph {
	private static int LOGADD_PRECISION = 10;
	private static int MAX_ITERATION = 6;
...
//input, test, threshold, smoothingP, tiRules, iteration, gemmaNew, graph
	...
	public void main(ArrayList input, ArrayList test, Double threshold,
			String tiRules, int EMiteration, HashMap gemmaNew, HashMap graph) {
		...
		MultigramGraph obj = new MultigramGraph();

		Iterator itToken = input.iterator();
		while (itToken.hasNext()) {
			obj.tokenize((String) itToken.next());
		}
...
//		for (Double i = 0.0; i <= 0.09; i = i + 0.01) {
		for (Double i = 0.0; i < 0.01; i = i + 0.01) {
			...
			obj.printTranslationScore(input, obj.translationNew, i);
		}
		
		if(tiRules != ""){ // if user provide a file to print transliteration rules with their probabilities
			obj.printGemma(gemmaNew, tiRules);
		}
//
...
//		}
		...
		// print multigrams with prob
		// print TranslateNew scores
		// print likelihood
		// print lambda
	}
}