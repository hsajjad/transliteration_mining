...
public class MultigramGraphSupervised_TrigramFully {
	...
	private double calcTrigramTranslatWordProb(String source, HashMap trigramTranslatProb )
	{
		...
		String curr = "", prev0 = "<s>", prev1 = "";

		for(int i = 1; i < source.length(); i++){
			curr = Character.toString(source.charAt(i));
			prev1 = Character.toString(source.charAt(i-1));
	...
	public void main(ArrayList test, Double threshold, int tiRules, int semiIteration, ArrayList seed, HashMap gemmaNew,
		...
		MultigramGraphSupervised_TrigramFully obj = new MultigramGraphSupervised_TrigramFully();
		...
		for (int i = 0; i < 1; i++) { // EM loop
			...
			while (itInput.hasNext()) {
				...
				//				obj.translatProbNorm(inputNum, obj.alphaTotal, INI); // for seed data
		...
		for (int i = 0; i < semiIteration; i++) { // EM loop
			...
			alphaBC = new HashMap(); // for pair is b,c,a. no of ti unit trigram types where bc is the first two elements
			trigramMultigram = new HashMap(); // frequency of transliteration unit pair (a,b,c) in the aligned seed corpus
			...
			else
			{
				temp = obj.semiSupervisedSeedBigram(seed, gemmaSeed,
						obj.translationNew, 0, alphaB, bigramMultigram, unigramMultigram, trigramMultigram, alphaBC);	
			}
		...
		gemmaSeed.put("<ss>", boundaryLogProb);
		...
		obj.trigramSmoothedEstimates(gemmaInput, gemmaSeed, alphaBC, trigramMultigram, bigramMultigram, aGivenb, aGivenbc);
		bigramBackoffEstimates(gemmaSeed, alphaBC, backoffEstimateBC,bigramMultigram, aGivenbc);

		gemmaNew = new HashMap();
		//gemmaNew.putAll(gemmaInput);
		gemmaNew.putAll(gemmaSeed);

		double tm = Math.pow(10, obj.LAMBDA);
		tm = tm*1;
		//semiTest_Trigram testObj = new semiTest_Trigram();
		...
		//		tiRules, charAlign, aGivenb, backoffEstimateB, obj.LAMBDA, seed, aGivenbc, backoffEstimateBC,
		//		trigramTranslatTrainProb);
		
		semiTest_Trigram_supervised testObj = new semiTest_Trigram_supervised();
		testObj.main(seed, gemmaNew, graph, -500.0, test, translationTest1, totalEdgesInTrain,
				tiRules, charAlign, aGivenb, backoffEstimateB, obj.LAMBDA, seed, aGivenbc, backoffEstimateBC,
				trigramTranslatTrainProb, totalFreq);
