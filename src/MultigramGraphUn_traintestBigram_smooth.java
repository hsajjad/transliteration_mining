...
public class MultigramGraphUn_traintestBigram_smooth {
	...
	public void main(ArrayList input, ArrayList test, Double threshold,
		...
		MultigramGraphUn_traintestBigram_smooth obj = new MultigramGraphUn_traintestBigram_smooth();
		...
		while (itToken.hasNext()) {
			...
			StringTokenizer st = new StringTokenizer (srcTar, "\t", false);
			String source = st.nextToken();
			String target  = st.nextToken();
		...
		obj.translatCharLogProb(); //calculates probability of each character
		...
		//semiTest testObj = new semiTest();
		//testObj.main(input, gemmaNew, graph, -500.0, test, translationTest1,totalEdgesInTrain, tiRules, charAlign, obj.LAMBDA, threshold);
		...
		String temp = obj.semiSupervisedBigram(input, gemmaNew, obj.translationNew, input.size(), alphaB, bigramMultigram, unigramMultigram);
		...
		obj.unigramBackoffEstimates(gemmaNew, alphaB, backoffEstimateB, aGivenb, unigramMultigram);

		test_Bigram_smooth testObj = new test_Bigram_smooth();
		testObj.main(input, gemmaNew, graph, -500.0, test, translationTest1, totalEdgesInTrain, tiRules, charAlign,
				aGivenb, backoffEstimateB, obj.LAMBDA, totalFreqAA);
	}
	
	private void unigramBackoffEstimates(HashMap gemmaNew, HashMap alphaB, HashMap backoffEstimateB, 
			HashMap aGivenb, HashMap unigramMultigram)
	{
		// alphaB / freqB + alphaB
	...
	private void bigramSmoothedEstimates(HashMap gemmaNew, HashMap alphaB,
		...
		while (it2.hasNext()) {
			...
			//Double freqB = Math.log10((Double) unigramMultigram.get(tiUnits[0]));
			// into frequencies
	...
	public String semiSupervisedBigram(ArrayList input, HashMap gemmaNew,
