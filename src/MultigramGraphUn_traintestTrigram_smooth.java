...
public class MultigramGraphUn_traintestTrigram_smooth {
...
//input, test, threshold, smoothingP, tiRules, iteration, gemmaNew, graph
	...
	public void main(ArrayList input, ArrayList test, Double threshold,
		...
		MultigramGraphUn_traintestTrigram_smooth obj = new MultigramGraphUn_traintestTrigram_smooth();
	...
	// obj.translation = new HashMap();
		...
		if(nontiOrder == 3)
		{
			o.test(test, translationTest1, 3);
		//	o.test(input, obj.translation, 3);	
		}
		//printingHashNum(translationTest1, "trigram");
		
		//*************************smoothed translation model ends********************//
		double totalFreqAA = 0.0;
		...
		for (int i = 0; i < EMiteration; i++) { // EM loop
			...
			totalFreqAA = obj.maximization(gemmaNew, Math.log10(input.size()));
		...
		String temp = obj.semiSupervisedTrigram(input, gemmaNew,
		...
		obj.bigramSmoothedEstimates(gemmaNew, alphaB, bigramMultigram, unigramMultigram, aGivenb);
		obj.unigramBackoffEstimates(gemmaNew, alphaB, backoffEstimateB, aGivenb);
		//printHash(aGivenb, "aGivenb");
		obj.trigramSmoothedEstimates(gemmaNew, alphaBC, trigramMultigram, bigramMultigram, aGivenb, aGivenbc);
		obj.bigramBackoffEstimates(gemmaNew, alphaBC, backoffEstimateBC,bigramMultigram, aGivenbc);
		
		test_Trigram_smooth testObj = new test_Trigram_smooth();
		testObj.main(input, gemmaNew, graph, -500.0, test, translationTest1,totalEdgesInTrain, tiRules, charAlign, 
				aGivenb, backoffEstimateB, obj.LAMBDA, aGivenbc, backoffEstimateBC, totalFreqAA);
		
	}
	
	private void unigramBackoffEstimates(HashMap gemmaNew, HashMap alphaB, HashMap backoffEstimateB, HashMap aGivenb)
	{
		// alphaB / freqB + alphaB
		...
		while(it.hasNext()){
			...
			double freqB = totalFreq +  (Double) gemmaNew.get(pair[0]);
	...
	private void bigramBackoffEstimates(HashMap gemmaNew, HashMap alphaBC, HashMap backoffEstimateBC,
	...
	private void bigramSmoothedEstimates(HashMap gemmaNew, HashMap alphaB,
			HashMap bigramMultigram, HashMap unigramMultigram, HashMap aGivenb){
		...
		while (it2.hasNext()) {
			...
			Double freqB = totalFreq + (Double) gemmaNew.get(tiUnits[0]); // convert prob
			
			//Double k = (Double) unigramMultigram.get(tiUnits[0]);
			//Double kLog  = Math.log10(k);
			//kLog = kLog*1;
			// into frequencies
			Double probA = (Double) gemmaNew.get(tiUnits[1]);
	...
	private void trigramSmoothedEstimates(HashMap gemmaNew, HashMap alphaBC,
	...
	public String semiSupervisedTrigram(ArrayList input, HashMap gemmaNew,
	...
	public int multigramsInViterbi(HashMap gDepth, HashMap multigrams, HashMap bigramMultigrams, HashMap unigramMultigrams,
			int totalTokens) {
		Object[] key = gDepth.keySet().toArray();
		Arrays.sort(key);
