...
public class MultigramGraphUn_traintest {
...
//input, test, threshold, smoothingP, tiRules, iteration, gemmaNew, graph
	...
	public void main(ArrayList input, ArrayList test, Double threshold,
		...
		MultigramGraphUn_traintest obj = new MultigramGraphUn_traintest();
...
//		Iterator tplus = trainPlusTest.keySet().iterator();
		...
		System.out.println("Final Lambda = " + Math.pow(10, obj.LAMBDA));
		
		decode deObj = new decode();
		deObj.decoderMain(test, gemmaNew);
		
		semiTest testObj = new semiTest();
		System.out.println("Using nontransliteration for order 1");
		testObj.main(input, gemmaNew, graph, -500.0, test, obj.translationTest,
				totalEdgesInTrain, tiRules, charAlign, obj.LAMBDA, threshold);
		
		
		
		// it will make an object of decoder and call its function to precess multgraims by sending multigramFile as parameter
