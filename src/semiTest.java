...
public class semiTest {
	...
	public double printViterbiProb(HashMap gDepth, String pair) {
		Object[] key = gDepth.keySet().toArray();
		Arrays.sort(key);
		...
		double length = (pairs[0].split("")).length + (pairs[0].split("")).length - 3;
		
		
		double normalizeViterbi = vitProb - Math.log10(length);
		return normalizeViterbi;
	}
	...
	public void main(ArrayList input, HashMap gemmaNew, HashMap graph, Double alphaWB, ArrayList test,
		...
		ArrayList<String> saveAlphaTotal = new ArrayList<String>();
		...
		Iterator itToken = test.iterator();
			...
			while (itInput.hasNext()) {
				...
				// print viterbi of each pair
				obj.calcViterbi(obj.graphDepth);
				double viterb = obj.printViterbiProb(obj.graphDepth, (String) test.get(inputNum));
				saveAlphaTotal.add(test.get(inputNum) + "\t" + viterb);
		...
		if(tiRules == 1){ // if user provide a file to print transliteration rules with their probabilities
			...
			writeToFile(saveAlphaTotal, "srcTotrgOrg");
	}
