...
public class MultigramGraphUn_traintestBigram {
	...
	public void main(ArrayList input, ArrayList test, Double threshold,
			int tiRules, int EMiteration, HashMap gemmaNew, HashMap graph, int charAlign, int nontiOrder) {
		...
		MultigramGraphUn_traintestBigram obj = new MultigramGraphUn_traintestBigram();
		...
		for (int i = 0; i < EMiteration; i++) { // EM loop
			...
			obj.maximization(gemmaNew, Math.log10(input.size()));
		...
		/* 
		 * bigram calculation from the viterbi of the training strings
		 * 
		 * */
		...
		String temp = obj.semiSupervisedBigram(input,gemmaNew, obj.translationNew, 0, alphaB, bigramMultigram, unigramMultigram);
		...
		obj.bigramTransliterationProbability(gemmaNew, bigramMultigram, unigramMultigram, aGivenb, alphaB);
		
		test_Bigram_unsmooth testObj = new test_Bigram_unsmooth();
		//testObj.main(input, gemmaNew, graph, test, obj.translationTest, tiRules, charAlign, obj.LAMBDA, threshold, aGivenb);
		...
		if(nontiOrder == 2)
		{
			testObj.main(input, gemmaNew, graph, test, bigramTranslatWordProb, tiRules, charAlign, obj.LAMBDA, threshold, aGivenb);	
		}
		if(nontiOrder == 3)
		{
			System.out.println("Using nonTi unigram");
			testObj.main(input, gemmaNew, graph, test, obj.translationTest, tiRules, charAlign, obj.LAMBDA, threshold, aGivenb);
		}
	...
	public String semiSupervisedBigram(ArrayList train, HashMap gemmaNew,
	...
	//	}
		...
		inputNum = 0;
		Iterator itInput = train.iterator();
		...
		while (itInput.hasNext()) {
			...
			//translatProbNorm(inputNum, alphaTotal, LAMBDA,train);
			//calcBeta(graphDepth);

			//calcGemma(graphDepth, gemmaNew, alphaTotal, inputNum);
