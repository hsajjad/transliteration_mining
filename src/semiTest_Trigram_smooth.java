...
public class semiTest_Trigram_smooth {
	...
	private void trigramViterbiProb(HashMap gemmaOld, String viterbiStr, HashMap aGivenb, HashMap backoffEstimatesB
			, HashMap aGivenbc, HashMap backoffEstimatesBC){

		String [] viterbiArr = ("<ss>NNN" + viterbiStr + "NNN<-s>" + "NNN<-ss>").split("NNN");
		viterbiArr[1] = "<s>";

		double viterbiProb = 0.0;
		String prev0 = "<s>", prev1 ="<s>";
		String curr = "";

		for(int i =2; i < viterbiArr.length; i++){
			prev0 = viterbiArr[i-2];
			prev1 = viterbiArr[i-1];
			curr = viterbiArr[i];

			String pair = prev0 + "NNN" + prev1 + "NNN" + curr;
			String pairBigram = prev0 + "NNN" + prev1;

			if(aGivenbc.containsKey(pair)){
				viterbiProb = viterbiProb + (Double)  aGivenbc.get(pair);
			}
			else
			{
				if(backoffEstimatesBC.containsKey(pairBigram) && aGivenb.containsKey(prev1 + "NNN" + curr)){
					viterbiProb = viterbiProb + 
					(Double) backoffEstimatesBC.get(pairBigram) + (Double) aGivenb.get(prev1 + "NNN" + curr);	
				}
				else
				{
					if(aGivenb.containsKey(prev1 + "NNN" + curr))
					{
						viterbiProb = viterbiProb + 
						(Double) aGivenb.get(prev1 + "NNN" + curr) ;

					}
					else
					{
						if(backoffEstimatesB.containsKey(prev1))
						{
							viterbiProb = viterbiProb + 
							(Double)  backoffEstimatesB.get(prev1) + (Double)  gemmaOld.get(curr);	
						}
						else
						{
							viterbiProb = viterbiProb + (Double)  gemmaOld.get(curr);							
	...
	private void trigramViterbiProbOld(HashMap gemmaOld, String viterbiStr, HashMap aGivenb, HashMap backoffEstimatesB
			, HashMap aGivenbc, HashMap backoffEstimatesBC){
		String [] viterbiArr = viterbiStr.split("NNN");
		...
		for(int i =2; i < viterbiArr.length; i++){
			if(i == 2)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							//if(backoffEstimatesB.containsKey(curr))
							if(backoffEstimatesB.containsKey(prev1))
							{
								viterbiProb = viterbiProb + 
								(Double)  backoffEstimatesB.get(prev1) + (Double)  gemmaOld.get(curr);	
			...
			if(i == viterbiArr.length-1)
			{
				...
				pair = prev0 + "NNN" + prev1 + "NNN" + curr;
				pairBigram = prev0 + "NNN" + prev1;	
	...
	public void main(ArrayList input, HashMap gemmaNew, HashMap graph, Double alphaWB, ArrayList test,
			...
			HashMap backoffEstimates, Double lambda, ArrayList seed, HashMap aGivenbc, HashMap backoffEstimateBC,
			HashMap translationTrain, Double totalFreq) {

		// take transition probability and value of nodes from the trained system
		graph = new HashMap();
		ArrayList viterbiArray = new ArrayList();
		ArrayList alphaArray = new ArrayList();

		lambda = Math.log10(0.5); // reestimating for test
		...
		semiTest_Trigram_smooth obj = new semiTest_Trigram_smooth();
		obj.translation.putAll(translationTest);

		inputNum = 0;

		//		printHash(gemmaNew, "gemmaNewTrigram");
		...
		while(flag)
		{
			...
			while (itInput.hasNext()) {
				...
				obj.eg.put("<ss>", gemmaOld.get("<ss>"));
				obj.eg.put("<s>", gemmaOld.get("<s>"));
				obj.eg.put("<-s>", gemmaOld.get("<-s>"));
				obj.eg.put("<-ss>", gemmaOld.get("<-ss>"));

				//				obj.calcAlpha(obj.graphDepth, aGivenb);
				//				printHash(obj.eg, "gemmaNewTrigram");
				//String viterbiString = obj.calcViterbiStr(obj.graphDepth) + "NNN<-s>";
				String viterbiString = obj.calcViterbiStr(obj.graphDepth);

				// calculate trigram transliteration probability for the viterbi string and use that value as alphaTotal
				// this function also handles boundary multigrams
				obj.trigramViterbiProb(gemmaOld, viterbiString, aGivenb, backoffEstimates, aGivenbc, backoffEstimateBC);
				obj.translatProbNorm(inputNum, obj.alphaTotal, lambda);

				//viterbiStrArray(src_tar, obj.graphDepth, viterbiArray);
				//alphaStrArray(src_tar, obj.alphaTotal, alphaArray);
		...
		//			obj.predictSeed(gemmaOld, seed, input.size(), aGivenb, backoffEstimates, lambda, translationTrain, aGivenbc, backoffEstimateBC);

		/*Output out = new Output();
			out.printOutput("", false, "alphaScore");
			//Iterator va = viterbiArray.iterator();
			Iterator va = alphaArray.iterator();
			while(va.hasNext())
			{
				String str = (String) va.next();
				out.printOutput(str, true, "alphaScore");
			}*/
	...
	private void alphaStrArray(String src_tar, double alph, ArrayList alphaArray)
	{
		Double alphaProb = alph / src_tar.length()-1;
		alphaArray.add(src_tar + "\t" + Math.pow(10, alphaProb));
	}
	...
	private void predictSeed(HashMap gemmaOld, ArrayList seed, int inputSize, HashMap aGivenb,
			HashMap backoffEstimates, double lambda, HashMap translationTrain, HashMap aGivenbc,
			HashMap backoffEstimateBC)
	{
		int i =0;
		try {
			BufferedWriter out = new BufferedWriter(
			...
			while (itInput.hasNext()) {
				...
				eg.put("<-s>", gemmaOld.get("<-s>"));
				eg.put("<-ss>", gemmaOld.get("<-ss>"));

				//			obj.calcAlpha(obj.graphDepth, aGivenb);

				//String viterbiString = obj.calcViterbiStr(obj.graphDepth) + "NNN<-s>";
				String viterbiString = calcViterbiStr(graphDepth);

				// calculate trigram transliteration probability for the viterbi string and use that value as alphaTotal
				// this function also handles boundary multigrams
				trigramViterbiProb(gemmaOld, viterbiString, aGivenb, backoffEstimates, aGivenbc, backoffEstimateBC);
