...
public class MultigramGraphSemi_Trigram {
...
private void trigramTranslationProbability(HashMap translationBigram, HashMap translationTrigram, HashMap TrigramTranslatProb){
	...
	public void main(ArrayList input, ArrayList test, Double threshold,
		...
		MultigramGraphSemi_Trigram obj = new MultigramGraphSemi_Trigram();
...
//			o.test(inputSeed, obj.translation, 1);	
		}
		if(nontiOrder == 2)
		{
...
*/		
		...
		for (int i = 0; i < semiIteration; i++) { // EM loop
			...
			if(i+1 < semiIteration){ // last iteration do bigram stuff as well
				temp = obj.semiSupervisedSeed(seed, gemmaSeed,obj.translationNew, input.size());
			}
			else
			{
	...
	//	obj.printHash(aGivenb, "ab");
	//	obj.printHash(aGivenbc, "abc");
	//	obj.printHash(alphaB, "alphab");
	//	obj.printHash(alphaBC, "alphabc");
		
		semiTest_Trigram_smooth testObj = new semiTest_Trigram_smooth();
		//testObj.main(input, gemmaNew, graph, alphaWB, test, obj.translationTest, totalEdgesInTrain, tiRules, charAlign, aGivenb);
		testObj.main(input, gemmaNew, graph, alphaWB, test, translationTest1, totalEdgesInTrain,
