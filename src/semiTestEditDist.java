...
public class semiTestEditDist {
	...
	public void printViterbiAlignments(HashMap gDepth) {
		...
		//obj.printOutput("" + str, Boolean.TRUE, "viterbiAlignments");
		obj.printOutput("" + vitStr, Boolean.TRUE, "viterbiAlignmentsUnigramED");
	}
	...
	public void main(ArrayList input, HashMap gemmaNew, HashMap graph, Double alphaWB, ArrayList test,
			HashMap translationTest, int totalEdgesInTrain, int tiRules, int charAlign, Double lambda, Double threshold,
			ArrayList seed) {
		...
		semiTestEditDist obj = new semiTestEditDist();
		...
		Iterator itToken = test.iterator();
			...
			average = average / ((double) seed.size());
			System.out.println("seed smallest" + smallest);
			
			try {
				BufferedWriter out = new BufferedWriter(
						new FileWriter("mineWPUnigramED"));
				
							
			Iterator itTestED = testEditDistance.keySet().iterator();
			while (itTestED.hasNext()) {
				Integer key = (Integer) itTestED.next();
				String str = (String) test.get(key) + "\t" + testEditDistance.get(key);
				
				//if(testEditDistance.get(key) >= smallest)
				//if(testEditDistance.get(key) >= average)
				//{
					out.write(str);
					out.write('\n');
