...
public class MultigramGraphSemi_traintest_Bigram {
	...
	public void main(ArrayList input, ArrayList test, Double threshold,
		...
		MultigramGraphSemi_traintest_Bigram obj = new MultigramGraphSemi_traintest_Bigram();
...
//			o.test(inputSeed, obj.translation, 2);	
		...
		if(nontiOrder == 3)
		{
			System.out.println("nonti order 3 for order 2 not possible. using nontransliteration of order 1");
			o.test(test, translationTest1, 1);
//			o.test(inputSeed, obj.translation, 1);	
		}
...
*/		
		...
		for (int i = 0; i < semiIteration; i++) { // EM loop
			...
			else
			{
				temp = obj.semiSupervisedSeedBigram(seed, gemmaSeed,
						obj.translationNew, input.size(), alphaB, bigramMultigram, unigramMultigram);	
			}
		...
		obj.unigramBackoffEstimates(gemmaSeed, alphaB, totalFreq, backoffEstimateB, aGivenb, unigramMultigram);
		...
		//testObj.main(input, gemmaNew, graph, alphaWB, test, translationTest1, totalEdgesInTrain,
			//		tiRules, charAlign, aGivenb, backoffEstimateB, obj.LAMBDA, seed, bigramTranslatTrainWordProb);
		semiTest_Bigram1_smooth testObj = new semiTest_Bigram1_smooth();
		testObj.main(input, gemmaNew, graph, alphaWB, test, translationTest1, totalEdgesInTrain,
		...
		for (Double i = 0.0; i < 0.01; i = i + 0.01) {
			//System.out.println(i);
	//		obj.printTranslationScore(inputSeed, obj.translationNew, i, alphaWB);
		}
	...
	private void unigramBackoffEstimates(HashMap gemmaSeed, HashMap alphaB, double totalFreq,
			HashMap backoffEstimateB, HashMap aGivenb, HashMap unigramMultigram)
	{
		// alphaB / freqB + alphaB
